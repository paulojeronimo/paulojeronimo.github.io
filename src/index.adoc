...
****
I have *over three decades (since 1993) of working experience in
{uri-pj-sitemap}#tags[several technologies^]* in the corporate market, from
startups to big companies.

My *[.underline]#Git repositories, Slides, Blog Posts, Articles, Tutorials,
Wikis, Videos, and Running Applications#* are centralized on one single page.
Open it through this button:

[.text-center]
{uri-pj-sitemap}[icon:sitemap[3x]]
****

.[red]#Do you need *my help with your projects*#?
[%collapsible%open]
====
Right now, *I am {uri-open-to-job}[open to job opportunities^]*.
*My résumés (CVs):* +
...
.[red]#Do you *want buy my ebooks*#?
[%collapsible]
====
These are the [.underline]#*ebooks* that I am currently developing# (_the real
names of them aren't already defined_):

. *{uri-tpttpa-ebook}[The Phi Treadmill Training Plan App (TPTTPA)]*. +
A full-stack wellness project to show you the integrated used of some modern technologies. +
_Main techs: Python, TypeScript, AWS_.
. *{uri-bash-tips-ebook}[Bash Tips]*.
. *{uri-fibonacci-ebook}[Programming The Fibonacci Sequence]*.
====
